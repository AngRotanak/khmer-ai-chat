...
type NodeListProps = Readonly<{
  nodes: (typeof AVAILABLE_NODES[number] & {
    __meta: typeof NODES_METADATA[BuilderNodeType]
    __enabled: boolean
  })[]
  onNodeAdd: (type: BuilderNodeType) => void
  showFeatureBlock?: boolean // ✅ new
}>


export default function NodeList({ nodes, onNodeAdd, showFeatureBlock }: NodeListProps) {
  const visibleNodes = showFeatureBlock
  ? nodes
  : nodes.filter(n => n.type !== 'feature-block')
