...
export function ActivatePageEditor() {
  ...
  const [newPageToken, setNewPageToken] = useState('')   // ✅ new state for token
  ...
  const loadPages = async () => {
    ...
    const loaded = await Promise.all(
      Object.entries(data).map(async ([id, value]) => {
        const flowRef = ref(db, `khmer-ai-chat/pages/${id}/flow`)
        const flowSnap = await get(flowRef)
        const hasFlow = flowSnap.exists()

        // ✅ read status from global meta branch instead of admin branch
        const metaStatusRef = ref(db, `khmer-ai-chat/pages/${id}/meta/status`)
        const metaStatusSnap = await get(metaStatusRef)
        const globalStatus = metaStatusSnap.exists() ? metaStatusSnap.val() : 'draft'

        const autoStatus = hasFlow ? globalStatus : 'disabled'

        if (autoStatus !== globalStatus) {
          // ✅ update meta/status so global stays in sync
          await set(metaStatusRef, autoStatus)
        }
  ...
  const handleAddPage = async () => {
    ...
    // ✅ Admin branch
    const pageList = ref(db, `khmer-ai-chat/admins/${user.id}/pages/${id}`)
    await set(pageList, {
    ...
    // ✅ Page → admin map
    const mapRef = ref(db, `khmer-ai-chat/page_admin_map/${id}`)
    await set(mapRef, user.id)

    // ✅ Global page branch
    const pageRef = ref(db, `khmer-ai-chat/pages/${id}`)
    await set(pageRef, {
      name: newPageName || `Page ${id}`,
      admin_id: user.id,
      status: newPageStatus,
    ...
    // ✅ Meta branch with token
    const metaRef = ref(db, `khmer-ai-chat/pages/${id}/meta`)
    await set(metaRef, {
      page_name: newPageName || `Page ${id}`,
      page_access_token: newPageToken || '',   // ✅ store token here
      createdAt: now,
      status: newPageStatus,
    })
  ...
  const handleRenamePage = async (id: string, newName: string) => {
    ...
    const nameRefAdmin = ref(db, `khmer-ai-chat/admins/${user.id}/pages/${id}/name`)
    const nameRefPage = ref(db, `khmer-ai-chat/pages/${id}/name`)
    const metaRef = ref(db, `khmer-ai-chat/pages/${id}/meta/page_name`)
    await Promise.all([set(nameRefAdmin, newName), set(nameRefPage, newName), set(metaRef, newName)])
    await loadPages()
  }
  ...
  const handleDeletePage = async (id: string) => {
    if (!user?.id) return
    await Promise.all([
      remove(ref(db, `khmer-ai-chat/admins/${user.id}/pages/${id}`)),
      remove(ref(db, `khmer-ai-chat/pages/${id}`)),
      remove(ref(db, `khmer-ai-chat/page_admin_map/${id}`)),
    ])
    await loadPages()
  }
            ...
            <div className="grid grid-cols-[1fr_auto] gap-4 items-start">
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {p.status === 'disabled' && (
                    <span className="text-yellow-400 text-xs whitespace-nowrap">⚠️ ផ្អាកដំណើរការបណ្តោះអាសន្ន</span>
                  )}
                </div>
                  ...
                  <>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const statusRef = ref(db, `khmer-ai-chat/pages/${p.id}/meta/status`)
