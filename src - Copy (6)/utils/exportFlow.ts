...
export function buildFlowExport_old(
  nodes: Node[],
  edges: Edge[],
  ...
  for (const block of validBlocks) {
    ...
    if (raw_type === "conversation-agent") {
      ...
      feature_blocks_by_type[raw_type][block_id] = {
        ...
        escape_keywords: Array.isArray(data.escape_keywords) ? data.escape_keywords : [],  // ✅ here
        ...
        sub_intents: Array.isArray(data.sub_intents)
          ? data.sub_intents.map((si: any) => ({
            ...
            escape_keywords: Array.isArray(si.escape_keywords) ? si.escape_keywords : [],  // ✅ here too
    ...
    // ✅ Only add entry_condition if it has a non-empty value
    if (data.entry_condition && typeof data.entry_condition === "object") {
      const val = (data.entry_condition as any).value;

      const hasValue =
        (Array.isArray(val) && val.some(v => typeof v === "string" && v.trim() !== "")) ||
        (typeof val === "string" && val.trim() !== "");

      if (hasValue) {
        block2.entry_condition = data.entry_condition;
      }
    }

    // ✅ Only add entry_detection_mode if non-empty
    if (typeof data.entry_detection_mode === "string" && data.entry_detection_mode.trim() !== "") {
      block2.entry_detection_mode = data.entry_detection_mode;
    }

    // ✅ Only add expected_intent if non-empty
    if (typeof data.expected_intent === "string" && data.expected_intent.trim() !== "") {
      block2.expected_intent = data.expected_intent;
    }

    feature_blocks_by_type[raw_type][block_id] = block2;
...
// 🔧 Build a Messenger-safe export payload from nodes and edges
export function buildFlowExport_new(
  nodes: Node[],
  edges: Edge[],
  currentUserId: string,
) {
  ...
  function normalizePayload(payload: any): string {
    ...
    if (typeof payload === 'object') {
      ...
      if (nodeId.startsWith(type + '.')) {
        console.log("normalizePayload → already normalized:", nodeId);
        return nodeId;
      }

      const result = `${type}.${nodeId}`;
      console.log("normalizePayload → constructed:", result);
      return result;
    }
    return '';
  }
  ...
  for (const block of validBlocks) {
    ...
    const isReferencedOnly = !edges.some(e => e.source === block.id);
    const isEntryBlock = typeof data.entry_trigger === "string" && data.entry_trigger.trim() !== "";
    if (isTemplateOnly && isReferencedOnly && !isEntryBlock) continue;

    // Normalize canvas and extract Messenger paths
    data.canvas = normalizeCanvasPathsSafely(data.canvas);

    // ✅ Use full canvas node list, not just feature nodes
    const allCanvasNodes = nodes;

    console.log("All canvas nodes for block", block_id, ":", allCanvasNodes.map(n => n.id));



    const { paths } = exportFeatureBlockMessenger(data, allCanvasNodes);
    const { layout } = parseCanvasSafely(data.canvas ?? {});
    ...
    // Include referenced template nodes
    for (const p of orderedPaths) {
      const normalizedPayload =
        typeof p?.payload === "string" && p.payload.includes(".")
          ? p.payload // already Messenger-safe
          : normalizePayload(p?.payload);

      const ref = (p?.template_ref && p.template_ref !== "unknown")
        ? p.template_ref
        : normalizedPayload;


      if (ref) {
        const targetNode = allCanvasNodes.find(
          n => n.id === ref || n.data?.block_id === ref || `${n.type}.${n.id}` === ref
        );
        if (targetNode) {
          blockNodes.push(targetNode);
          blockEdges.push({ id: `${block_id}-${ref}`, source: block_id, target: targetNode.id });
        }
      }
    }

    const edgeList: Edge[] = orderedPaths.map((p) => {
      const normalizedPayload =
        typeof p?.payload === "string" && p.payload.includes(".")
          ? p.payload
          : normalizePayload(p?.payload);
      ...
      if (!ref || !p?.id) return null;
      const targetNode = allCanvasNodes.find(
        n => n.id === ref || n.data?.block_id === ref || `${n.type}.${n.id}` === ref
      );
      if (!targetNode) return null;
      if (!blockNodes.some(n => n.id === targetNode.id)) blockNodes.push(targetNode);
      return {
        id: `${block_id}-${p.id}`,
        source: block_id,
        target: targetNode.id,
    ...
    // Assign full normalized canvas (with paths intact)
    feature_blocks_by_type[raw_type][block_id] = {
      block_id,
      block_name: data.block_name ?? block_id,
      block_type: raw_type,
      ...
      entry_trigger: data.entry_trigger ?? "message",
      intent_confidence: data.intent_confidence ?? "0.85",
      canvas: normalizedCanvas,   // ✅ keep full normalized canvas
      raw_canvas: stripUndefined({ nodes, edges }),
    };
  ...
  // 🧩 Extract shared templates using full canvas node list
  const { updatedBlocks, shared_templates } = extractTemplates(feature_blocks_by_type, nodes);
...
export function buildFlowExport(
  ...
  for (const block of validBlocks) {
    ...
    // After normalization, rewrite template_ref values
    normalizedCanvas.paths = (normalizedCanvas.paths ?? []).map(p => {
      const targetNode = nodes.find(
        n => n.id === p.template_ref || n.data?.block_id === p.template_ref
      );
      return {
        ...p,
        template_ref: formatTemplateRef(targetNode) ?? p.template_ref
      };
    });
...
export function extractValidBlocks_old(nodes: Node[], edges: Edge[]): Node[] {
  const valid: Node[] = [];
...
export function extractValidBlocks(nodes: Node[], edges: Edge[]): Node[] {
  ...
  for (const node of nodes) {
    ...
    const isSpecialBlock = [
      ...
      "smart-welcome",
      "carousel"
    ].includes(node.type ?? data.block_type);

    if (connectedIds.has(node.id) || isSpecialBlock) {
      valid.push(node);
    } else {
      console.warn(`⚠️ Node ${node.id} is not connected by any edge`);
    }
...
export function normalizeCanvasPathsSafely_old(canvas: any): any {
...
export function extractTemplates_old(
...
export function extractTemplates(
  ...
  function collectPayloadRefs(items: any[], context: string): string[] {
    ...
    for (const item of items) {
      ...
      if (ref && ref !== "unknown") {
        const [refType, refId] = ref.includes('.') ? ref.split('.', 2) : [null, ref];
  ...
  for (const [blockType, blocks] of Object.entries(feature_blocks_by_type)) {
    ...
    for (const [blockId, block] of Object.entries(blocks)) {
      ...
      // ✅ Rewrite template_ref values to type.id
      normalizedCanvas.paths = (normalizedCanvas.paths ?? []).map(p => {
        const targetNode = nodes.find(
          n => n.id === p.template_ref || n.data?.block_id === p.template_ref
        );
        return {
          ...
          template_ref: targetNode
            ? `${normalizeToTemplateType(
              targetNode.data?.block_type || targetNode.data?.template_type || targetNode.type
      ...
      while (safePaths.length > 0) {
        ...
        if (template_type === "generic-template") {
          ...
          console.log("✅ Added voice-template sharedTemplate:", sharedTemplates[key]);
          continue;
        } else {
...
export function extractTemplates_backup(
  feature_blocks_by_type: Record<string, any>,
  nodes: Node[]
) {
  const sharedTemplates: Record<string, SharedTemplate> = {};
