...
export type BlockContentType =
  ...
  | ButtonBlockData
  | VoiceBlockData
  | VoiceTemplateNodeData // ✅ add this
...
export type SharedTemplate = {
  ...
  config: {
    delay_seconds: number;
    emoji_style?: string;   // optional for voice-template
    tone: string;
    show_typing: boolean;
    priority?: string;      // voice-template specific
  };
  locales: Record<string, { lang: string; text?: string; cards?: any[]; media_url?: string }>;
};
...
export function normalizeToTemplateType(type: any): string {
  ...
  switch (type.trim()) {
    ...
    case "voice-template":
    case "voiceTemplate":
      // Internal node type for voice; Messenger delivery will normalize to "text-message"
