      ...
      nodes.map(node => {
        ...
        if (type === 'quick-menu') {
          safePaths = [{
            ...
            condition: { match: 'includes', value: '' }, // ✅ valid MatchType
            delay: { seconds: 0 },
          }];
        }
        else {
          // ✅ Normal path handling for other block types
            ...
            <div className="mt-2 flex flex-col">
              {data.canvas.paths.map(path => (
                <ButtonPath
                  key={path.id}
                  id={path.id}
                  label={`${path.label}: ${getTriggerLabel(path.trigger ?? 'immediate')}`}  // ✅ use path.label
