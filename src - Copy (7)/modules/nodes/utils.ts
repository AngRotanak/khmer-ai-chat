...
export function createNodeWithDefaultData(
  ...
  // ✅ Initialize Messenger templates for feature-block
  const defaultTemplates: Record<string, any> = {};
  if (type === 'feature-block') {
    for (const path of defaultPaths) {
      const baseId = path.template_ref;
      defaultTemplates[`${baseId}_en`] = { message: 'Default English message' };
      defaultTemplates[`${baseId}_kh`] = { message: 'សារដើម Default Khmer message' };
    }
  }
  ...
  const enrichedData =
    type === 'feature-block'
      ? {
          block_id: nodeId,
          block_name: '',
          block_type: (overrides as any)?.data?.block_type || 'info',
          is_active: true,
          tags: [],
          linked_pages: [],
          created_by: 'admin',
          ...
          config: {},
          templates: defaultTemplates,   // ✅ now has en + kh entries
          canvas: {
            layout: 'vertical',
            paths: defaultPaths
          },
  ...
  return {
    ...
    id: nodeId,
    position: overrides?.position ?? { x: 0, y: 0 }, // ✅ ensure position is set
    ...overrides,
  } as Node;
}
