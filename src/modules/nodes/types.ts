...
export interface PostData {
  ...
  comments: Comment[]   // strictly Comment[]
  clusters?: Cluster[]
  priority: 'normal' | 'high' | 'urgent'
...
export interface Conversation {
  ...
  timestamp: number
  routing: string
  priority: 'normal' | 'high' | 'urgent'   // ✅ restrict to known values
...
export interface TextMessageNodeData extends BaseNodeData {
  ...
  priority?: 'normal' | 'high' | 'urgent';   // ✅ added urgent here
...
export interface ConversationAgentNodeData extends BaseNodeData {
  ...
  expected_intent: string        // 👈 renamed from intent_id
  ...
  entry_priority: 'high' | 'normal' | 'low'
  ...
  end_message_en: string
  end_message_kh: string

  // Payload references
  fallback_payload: string
  end_conversation_payload: string
...
export interface SubIntentConfig {
  expected_intent: string        // 👈 renamed from id
  confidence_threshold: number
  flow_payload: string
  release_on_complete: boolean
  escape_keywords: string[]
...
export type SharedTemplate = {
  ...
  template_type: string;
  is_active: boolean;
  channel: 'messenger' | 'comment';   // ✅ add this
  config: {
    delay_seconds: number;
    emoji_style?: string;
    tone: string;
