...
import ChatWithAgentPropertyPanel from '~/modules/sidebar/panels/node-properties/property-panels/chat-with-agent-property-panel'
import { produce } from 'immer'
import { useReducer } from 'react'
import { useCanvasStore } from '~/stores/canvas-store'
import { isEmpty } from 'radash'

const NODE_TYPE = BuilderNode.CHAT_WITH_AGENT
type ChatWithAgentNodeProps = NodeProps<Node<any, typeof NODE_TYPE>>

export function ChatWithAgentNode({ id, isConnectable, selected, data }: ChatWithAgentNodeProps) {
                    ...
                    <div className="flex flex-col p-4">
                        <div
                            className={cn(
                                ...
                                isEmpty(data.welcome_message_kh) && isEmpty(data.welcome_message_en) ? 'mt-1' : ''
                            )}
                        >
                            {isEmpty(data.welcome_message_kh) && isEmpty(data.welcome_message_en) ? (
                                <span className="text-light-900/80 italic">No welcome message yet...</span>
                            ) : (
                                <>
                                    {data.welcome_message_kh && (
                                        <div className="text-light-100">{data.welcome_message_kh}</div>
                                    )}
                                    {data.welcome_message_en && (
                                        <>
                                            <div className="h-2 border-t border-teal-300/40 my-2" />
                                            <div className="text-light-100/80 italic">{data.welcome_message_en}</div>
                                        </>
                                    )}
                            ...
                            {isEmpty(data.welcome_message_kh) && (
                                <div className="mt-1 text-xs text-red-400 italic">⚠️ Missing Khmer welcome message</div>
                            )}
                        </div>
                    </div>
...
export const metadata: RegisterNodeMetadata<any> = {
    type: 'chat-with-agent',
    node: memo(ChatWithAgentNode),
    detail: {
        icon: 'i-mdi:headset', // ✅ use a valid icon pack
    ...
    defaultData: {
        ...
        welcome_message_kh: '', // ✅ Khmer welcome message
        welcome_message_en: '', // ✅ English welcome message
    },
    propertyPanel: ChatWithAgentPropertyPanel, // ✅ now editable
}
