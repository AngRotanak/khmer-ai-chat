...
export function ConversationQueue({
  ...
  activeConversationId
    ...
    console.log("📥 ConversationQueue received conversations:", conversations.map(c => ({
    id: c.id,
    type: c.type,
  ...
  console.log("🛠️ Normalized conversations:", normalized.map(c => ({ id: c.id, priority: c.priority, lastMessage: c.lastMessage })))
 ...
 // Sort
  ...
  console.log("📊 Sorted conversations:", sorted.map(c => ({ id: c.id, ts: c.timestamp, priority: c.priority })))
  ...
  // 🔹 Hybrid search: user name, ID, lastMessage, status, priority
  // Filter
  ...
  console.log("🔎 Filtered conversations:", filtered.map(c => ({ id: c.id, lastMessage: c.lastMessage })))
  ...
  // Apply date filter
  if (dateFilter === "today") {
    filtered = filtered.filter(c => getDateBucket(c.timestamp) === "today")
  } else if (dateFilter === "yesterday") {
    filtered = filtered.filter(c => getDateBucket(c.timestamp) === "yesterday")
  } else if (dateFilter === "week") {
    const weekAgo = Date.now() / 1000 - 7 * 24 * 3600
    filtered = filtered.filter(c => c.timestamp >= weekAgo)
  }
  ...
  console.log("📦 Grouped conversations:", {
    today: grouped.today.map(c => c.id),
    yesterday: grouped.yesterday.map(c => c.id),
    older: grouped.older.map(c => c.id)
  })
      ...
      <div className="flex-grow overflow-y-auto scrollbar-dark-teal p-2 space-y-6">
        {["today", "yesterday", "older"].map(bucket => (
          grouped[bucket].length > 0 && (
            <div key={bucket}>
              ...
              {/* 🔹 Each conversation separated with space-y */}
                ...
                {grouped[bucket].map(conv => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg flex items-center gap-3 border border-dark-700 shadow-md 
                  ...
                  ${isActive
                    ...
                    >
                      {/* Avatar */}
                      {conv.avatar ? (
                        <img src={conv.avatar} alt={userName} className="h-10 w-10 rounded-full object-cover" />
                      ) : (
                        <div className="h-10 w-10 rounded-full bg-dark-600 flex items-center justify-center text-light-400">?</div>
                      )}
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-light-400">
            <span className="text-sm">No conversations match your filter…</span>
          </div>
        )}
      </div>
    </div>
