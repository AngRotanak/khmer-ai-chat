...
export async function sendCommentReply(
  ...
  pageId: string,
  userId: string,
  postId: string   // ✅ must come from activeConversation.id
) {
  if (!message.trim()) return
  try {
    console.log("sendCommentReply args:", { commentId, message, pageToken, pageId, userId, postId })
    ...
    console.log(`✅ Reply sent to Facebook for postId=${postId}`, result)
    ...
    console.log("✅ Reply stored in Firebase for postId=", postId, enrichedReply)
  } catch (err) {
