...
export const InfoPanel: React.FC<InfoPanelProps> = ({
  ...
  const [displayMode, setDisplayMode] = useState<'kh' | 'en' | 'both'>('both')
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-dark-300 rounded-md p-3 space-y-3 bg-dark-800 " >
        ...
        {/* 🔹 Priority Mapping */}
        <div>
          <label className="text-xs font-semibold text-light-900/60 mb-1 block">Priority on Match</label>
          <select
            value={data.entry_priority ?? "normal"}
            onChange={e => updateData({ entry_priority: e.target.value as "normal" | "high" | "urgent" })}
            className="h-7 w-full rounded-md bg-dark-800 text-light-100 text-xs border border-transparent px-1 outline-none transition hover:(bg-dark-300 border-teal-600)"
          >
          ...
          <p className="text-[10px] text-light-900/40 mt-1">
            If the keyword/intent condition matches, mark this incoming message as High or Urgent priority.
          </p>
        </div>
          ...
          <div>
            ...
            >
              ...
              {availableIntents.map(intent => {
                const kh = intent.display_name.kh
                const en = intent.display_name.en
                return (
        ...
        {Array.isArray(data.canvas?.paths) && data.canvas.paths.map((path, index) => {
          ...
          return (
            <div key={path.id} className="flex flex-col gap-2 border border-dark-300 dark:border-dark-700 rounded-md p-3 bg-dark-500 dark:bg-dark-600">
              ...
              {/* Detection Mode */}
              {['message', 'comment'].includes(data.entry_trigger) && path.trigger === 'condition' && (
                <div className="mb-2">
                  <label className="text-xs text-light-900/40 mb-1 block">Detection Mode</label>
                  <div className="flex gap-x-4 items-center">
                    <label className="flex items-center gap-x-1 text-xs text-light-100">
              ...
              {/* Expected Intent */}
              {mode === 'intent' && path.trigger === 'condition' && (
                <div className="mb-2">
                  <label className="text-xs text-light-900/40 mb-1 block">Expected Intent</label>
                  <select
              ...
              {['message', 'comment'].includes(data.entry_trigger) && mode === 'keyword' && path.trigger === 'condition' && (
                <div className="mb-2">
              ...
              {path && (() => {
                ...
                // Case 1: New unified string payload
                if (typeof payload === 'string') {
                  return (
                    <div className="mb-2">
                      <label className="text-xs text-light-900/40 mb-1 block">Payload (auto-linked)</label>
                      <input
                        type="text"
                        value={payload}
                        readOnly
                      ...
                      />
                      <p className="text-[10px] text-light-900/40 mt-1">
                        This payload is auto-linked to the target block.
                      </p>
                      <div className="text-[10px] mt-1 font-mono text-green-400">✅ Unified string payload</div>
                    </div>
                  );
                }

                // Case 2: Old object payload (Messenger-safe)
