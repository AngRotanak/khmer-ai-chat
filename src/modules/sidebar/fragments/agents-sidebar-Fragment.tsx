...
export function AgentSidebarFragment({
  ...
  const handleSend = (msg: any) => {
    if (!activeConversation) {
      alert("Please select a conversation before sending media.")
      return
    }
  ...
  return (
    <aside className="flex h-full shrink-0 bg-dark-400 dark:bg-dark-800">
      {/* Active panel area */}
      {activePanel !== 'none' && (
        <div className="w-96 h-full bg-dark-500 dark:bg-dark-900 overflow-y-auto">
      ...
      <div className="shrink-0 h-full flex flex-col gap-2 p-1.5 border-l border-dark-300">
        <SidebarButtonItem
