    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            ...
            if (secondsPassed % 5 === 0) {
                try {
                    const res = await fetch("https://bakongapi.example/check_transaction_by_md5", {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": `Bearer ${process.env.BAKONG_TOKEN}`, // ⚠️ unsafe in frontend
                        },
                        body: JSON.stringify({ md5, selected_package: selectedPackage }),
                    })
                    const data = await res.json()
