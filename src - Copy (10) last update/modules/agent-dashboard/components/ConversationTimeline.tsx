...
export function ConversationTimeline({
  ...
  useEffect(() => {
    if (scrollRef.current) {
      console.log("[🪵 Debug] Before scroll:", {
  ...
  }, [timeline]); // depend on timeline array
  ...
  return (
    <div className="w-full md:flex-grow flex flex-col h-full bg-dark-900">
      {/* Sticky Header */}
      <header className="sticky top-0 z-10 p-2 flex flex-wrap sm:flex-nowrap justify-between items-center border-b border-dark-600 bg-dark-900 gap-2">
      ...
      <div
        ref={scrollRef}
        className="flex-grow overflow-y-auto h-full p-2 space-y-4"
      >
        {timeline.length === 0 ? (
                  ...
                  >
                    ...
                    {typeof item.videoUrl === "string" && item.videoUrl.length > 0 && (
                      <video
                        src={item.videoUrl}
                        controls
                        className="rounded-lg mt-1 w-64 h-40 object-contain shadow-md"
                      />
                    )}
