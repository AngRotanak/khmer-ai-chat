  ...
  return featureBlocks.flatMap(block => {
    ...
    return paths.map((path, index) => {
      ...
      if (typeof path.payload === 'string') {
        ...
        if (typeof obj.payload_str === 'string' && obj.payload_str) {
          normalizedPayload = obj.payload_str;
        } else if (typeof obj.template_type === 'string' && typeof obj.node_id === 'string') {
          // ✅ Prevent double-prepending
          normalizedPayload = obj.node_id.startsWith(obj.template_type + '.')
            ? obj.node_id
            : `${obj.template_type}.${obj.node_id}`;
        } else if (typeof obj.node_id === 'string') {
          normalizedPayload = obj.node_id;
        } else {
          normalizedPayload = '';
        }
      }
