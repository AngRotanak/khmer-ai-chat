...
function formatRelativeAndReadable(timestamp: string | number) {
  ...
  if (tsNum < 1e12) tsNum = tsNum * 1000 // convert seconds → ms
...
export function ConversationQueueRow({
  conversation,
  onSelect,
  searchTerm,
}: ConversationQueueRowProps) {
  ...
  // Preview text: last message already normalized in queue listener
  ...
  // Channel detection based on unified schema
  const channelIcon = conversation.type === "comment" ? "📰" : "💬"
  const channelLabel = conversation.type === "comment" ? "Comment Thread" : "Messenger Chat"
  const channelBg =
  ...
  return (
    <div
      ...
      className={`p-3 border-b border-dark-600 cursor-pointer ${channelBg} ${
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <div className="flex flex-col">
            <span className="font-semibold text-light-100 flex items-center gap-1">
              {channelIcon} {highlightMatch(name, searchTerm || "")}
            </span>
            <span className="text-xs text-light-400">{formattedTime}</span>
          </div>
        </div>
        <span className="text-xs text-light-400">{channelLabel}</span>
      </div>
