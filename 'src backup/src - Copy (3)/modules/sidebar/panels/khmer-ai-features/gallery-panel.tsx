    ...
    <div className="p-4 bg-dark-800 rounded-lg space-y-4 relative">
      ...
      {/* <div className="flex gap-2 mb-3 flex-wrap">
        {['all', ...categories].map(c => (
          <button
            ...
            className={`px-3 py-1 rounded text-xs ${categoryFilter === c ? 'bg-teal-500 text-white' : 'bg-dark-600 text-light-100'
              }`}
          >
            {c}
          </button>
      ...
      {currentItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-dark-800 p-4 rounded-lg max-w-md w-full space-y-3">
            <h4 className="text-lg font-semibold text-light-100">Media Details</h4>

            {currentItem.media_type === 'image' && (
              <img src={currentItem.media_url} alt={currentItem.filename} className="w-full rounded" />
            )}
            {currentItem.media_type === 'video' && (
              <video src={currentItem.media_url} controls className="w-full rounded" />
            )}
            {currentItem.media_type === 'audio' && (
              <audio src={currentItem.media_url} controls className="w-full" />
            )}

            <div className="text-sm text-light-100"><strong>Filename:</strong> {currentItem.filename}</div>
            <div className="text-sm text-light-100"><strong>Category:</strong> {currentItem.category}</div>
            <div className="text-sm text-light-100"><strong>Size:</strong> {(currentItem.size / 1024).toFixed(1)} KB</div>
