...
export default function ButtonTemplatePropertyPanel({ id }: { id: string }) {
  ...
  if (!node || node.type !== 'button-template') {
    return <div className="text-red-500">⚠️ Invalid node</div>
  }
  const data = node.data as ButtonTemplateNodeData
  ...
  const updateOption = (index: number, patch: Partial<ButtonTemplateNodeData['options'][number]>) => {
