...
import type { Conversation, ConversationMessage, FlowLog, PostData } from '~/modules/nodes/types'
        ...
        setConversations(prev => {
          ...
          return [
            ...others,
            {
              ...
              type: conv.type, // "message" or "comment"
...
const handleSelectConversation = (conv: Conversation) => {
  ...
  // Reset active conversation with all fields
  ...
  const postsRef = ref(
    db,
    `khmer-ai-chat/pages/${currentPageId}/conversations/${conv.user_id}/posts`
  )
    ...
    const posts: PostData[] = Object.entries(postsData).map(([postId, postData]: any) => ({
      ...
      comments: Object.entries(postData.comments || {}).map(([commentId, c]: any) => ({
        ...
        replies: Object.entries(c.replies || {}).map(([rid, r]: any) => ({
          ...
          replies: [], // nested replies handled recursively if needed
        })),
      })),
    }))

    console.log("💬 Parsed posts with comments:", posts)

    _setActiveConversation(prev =>
      prev ? { ...prev, posts } : { ...conv, posts, messages: [], flowLogs: [] }
    )
  ...
  onValue(msgsRef, snapshot => {
    ...
    _setActiveConversation(prev =>
      prev ? { ...prev, messages: msgs } : { ...conv, messages: msgs, flowLogs: [], posts: [] }
    )
    setActiveConversation(prev =>
      prev ? { ...prev, messages: msgs } : { ...conv, messages: msgs, flowLogs: [], posts: [] }
    )
        ...
        <div className="flex flex-col flex-grow h-full bg-dark-900">
          ...
          <div className="sticky bottom-0 bg-dark-900 border-t border-dark-600">
            ...
            <ReplyBar
              ...
              postTitle={selectedPostTitle}       // ✅ track in state
              commentText={selectedCommentText}   // ✅ track in state
              ...
              onCancelReply={handleCancelReply}   // ✅ separate prop
            />
