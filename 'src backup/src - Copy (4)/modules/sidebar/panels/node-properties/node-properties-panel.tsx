...
export function NodePropertiesPanel() {
  ...
  const selectedPathId = useCanvasStore(s => s.selectedPathId)
...
const onNodeClick = (id: string) => {
  ...
  if (clickedNode) {
    ...
    // ✅ Auto-select first path if it's a feature-block
    if (clickedNode.type === 'feature-block') {
      const rawCanvas = (clickedNode as any).data?.canvas
      const { paths } = parseCanvasSafely(rawCanvas)
      const firstPathId = paths?.[0]?.id
      useCanvasStore.getState().setSelectedPathId(firstPathId)
    } else {
      useCanvasStore.getState().setSelectedPathId(undefined)
    }
  }
}
      ...
      >
        ...
        <Pane minSize={300}>
          ...
          <div className="h-full flex flex-col bg-dark-400 dark:bg-dark-900">
            ...
            <OverlayScrollbarsComponent className="grow" defer options={defaultOverlayScrollbarsOptions}>
              {selectedNode
                ? <NodePropertyPanel id={selectedNode.id} type={selectedNode.type} selectedPathId={selectedPathId} />
