...
export function FlowManagerPanel() {
  ...
  const handleSelectPage = async (pageId: string) => {
    ...
    if (!pageId) {
      setCurrentPageId('')
      setFlowData({})
      clearFlowList() // ✅ clear global flow list
      return
    }
    ...
    try {
      ...
      useCanvasStore.getState().setFlowData(data)
  ...
  const handleSaveFlow = async () => {
    ...
    // ✅ Sync and sanitize from React Flow before export
    ...
    try {
      // ✅ Build full export
      const exportData = buildFlowExport(syncedNodes, syncedEdges, 'full')

      // ✅ Simulate Messenger delivery (for preview/debug only)
      simulateMessengerDelivery(exportData)
      ...
      // ✅ Deep sanitize export payload
      const sanitizedExport = sanitizeCanvas(exportData)

      // ✅ Confirm overwrite if any top-level block already exists
      const flowRef = ref(db, `khmer-ai-chat/admins/${user.id}/pages/${currentPageId}/flow`)
      const snapshot = await get(flowRef)
      const existingFlows = snapshot.val() || {}

      const existingKeys = Object.keys(existingFlows)
      const newKeys = Object.keys(sanitizedExport)
      const overlapping = newKeys.filter(k => existingKeys.includes(k))
      ...
      // ✅ Save all blocks to Firebase
      await update(flowRef, sanitizedExport)

      setSaveStatus('saved')
      setTimeout(() => setSaveStatus('idle'), 3000)

      alert(`✅ Flow saved successfully! Exported blocks: ${Object.keys(sanitizedExport).join(', ')}`)
    } catch (err) {
