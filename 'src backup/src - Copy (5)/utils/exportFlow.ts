...
export function buildFlowExport(
  ...
  function normalizePayload(payload: any): string {
    ...
    if (typeof payload === 'object') {
      ...
      return type && nodeId ? `${type}.${nodeId}` : nodeId;
    }
    return '';
  }
  ...
  for (const block of validBlocks) {
    ...
    // ✅ Use the full node list passed into buildFlowExport
    ...
    for (const p of orderedPaths) {
      const normalizedPayload = normalizePayload(p?.payload);
...
export function normalizeCanvasPathsSafely(canvas: any): any {
  ...
  function normalizePayload(payload: any): string {
    ...
    if (typeof payload === 'string') {
      ...
      console.log("normalizePayload input:", payload, "output:", out);
      return out;
    }
    if (typeof payload === 'object') {
      const nodeId = typeof payload.node_id === 'string' ? payload.node_id : '';
  ...
  canvas.paths = paths.map((p: any) => {
    ...
    // Only set payload if normalization produced a valid string
...
export function extractTemplates(
  ...
  // ✅ Helper: normalize payloads safely
  function normalizePayload(payload: any): string {
    if (!payload) return '';
    if (typeof payload === 'string') {
      return (payload === 'unknown' || payload === 'unknown.unknown') ? '' : payload;
  ...
  function collectPayloadRefs(items: any[], context: string): string[] {
    ...
    for (const item of items) {
      ...
      if (ref && ref !== "unknown" && !sharedTemplates[ref]) {
        refs.push(ref);
      }
    }
    if (refs.length > 0) {
  ...
  for (const [blockType, blocks] of Object.entries(feature_blocks_by_type)) {
    ...
    for (const [blockId, block] of Object.entries(blocks)) {
      ...
      while (safePaths.length > 0) {
        const currentPath = safePaths.shift();
        const ref = currentPath?.template_ref || normalizePayload(currentPath?.payload);

        if (typeof ref !== "string" || !ref || processed.has(ref)) continue;
        processed.add(ref);

        const node = nodes.find(
          n => n.id === ref || n.data?.block_id === ref || `${n.type}.${n.id}` === ref
        );
        console.log("extractTemplates → following ref:", ref, "found node:", node);
