...
export function FlowManagerPanel() {
  ...
  const handleSelectPage = async (pageId: string) => {
    ...
    try {
      ...
      // Sort flows by name for readability
      flowItems.sort((a, b) => (a.name ?? '').localeCompare(b.name ?? ''));

      setFlowData(data);
      console.log("✅ setFlowData called with:", data);

      setFlowList(flowItems);

      console.log(`✅ Loaded ${flowItems.length} flows for page: ${pageId}`);
      console.log('✅ Flow list:', flowItems);
    } catch (err) {
                  ...
                  >
                    ...
                    <span
                      ...
                      onClick={() => {
                        ...
                        // Prefer raw_canvas for editor restore
                        ...
                        onClick={async e => {
                          ...
                          try {
                            ...
                            if (latestFlowData.feature_blocks_by_type?.[flow.type]) {
                              deletePayload[`feature_blocks_by_type/${flow.type}`] = Object.fromEntries(
                                blocks.map(id => [id, null])
                              );
                            }

                            // ✅ Delete referenced templates
                            for (const tpl of templates) {
                              deletePayload[`shared_templates/${tpl}`] = null;
                            }
...
function cloneBlock(oldBlock: any, newName: string) {
  const newBlock = JSON.parse(JSON.stringify(oldBlock));
  const newId = nanoid();
  ...
  // Update references inside raw_canvas, paths, etc.
  if (newBlock.raw_canvas?.nodes) {
    newBlock.raw_canvas.nodes.forEach((node: any) => {
      node.id = newId;
      node.data.block_id = newId;
