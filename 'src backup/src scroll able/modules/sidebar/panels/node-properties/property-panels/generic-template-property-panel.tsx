import type { GenericTemplateNodeData, BuilderNodeType } from '~/modules/nodes/types'
import { useState } from 'react'

type Props = Readonly<{
  id: string
  type: BuilderNodeType
  data: GenericTemplateNodeData
  updateData: (data: Partial<GenericTemplateNodeData>) => void
}>

export default function GenericTemplatePropertyPanel({ id, data, updateData }: Props) {
        ...
        <div className="flex flex-col gap-4">
          ...
          {card.options.map((opt, index) => (
            <div key={opt.id} className="flex flex-col gap-2 border border-dark-200 dark:border-dark-600 rounded-md p-3 bg-dark-400 dark:bg-dark-800">
              ...
              {/* Payload status */}
              {opt.payload?.startsWith('payload_') && (
                <div className="text-xs text-teal-500 mt-1">✅ Auto-linked</div>
              )}
              {opt.payload && !opt.payload.startsWith('payload_') && (
                <div className="text-xs text-yellow-400 mt-1">✍️ Manual payload</div>
              )}
