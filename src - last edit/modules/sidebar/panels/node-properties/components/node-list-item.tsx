...
type NodeListItemProps = Readonly<ComponentPropsWithoutRef<'button'> & {
  ...
  chainStatus?: 'valid' | 'disconnected' | 'unknown';
}>

export function NodeListItem({ id, title, className, icon, selected, pseudoSelected, chainStatus, ...props }: NodeListItemProps) {
  return (
    <button
      type="button"
      className={cn(
    ...
    >
     ...
     <div className="flex items-center gap-2">
        ...
        {chainStatus === 'valid' && (
          <span className="rounded-md bg-dark-100 px-2 py-1.5 text-2.5 text-gray-200/80 font-semibold leading-none tracking-wide">
            ✅
          </span>
        )}
