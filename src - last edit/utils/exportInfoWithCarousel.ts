...
export function exportInfoWithCarousel(
  ...
  for (const path of textPaths) {
    ...
    const isTargetActive = typeof target.data?.is_active === 'boolean' ? target.data.is_active : false

    if (message) {
      console.log(`💬 Extracted text-message: "${message}" from node "${target.id}"`)
      kh.push({ template_type: 'text', is_active: isTargetActive, text: message })
      en.push({ template_type: 'text', is_active: isTargetActive, text: message })
    }
  }
