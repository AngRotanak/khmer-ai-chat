...
export function exportFeatureBlockMessenger(
  ...
  for (let i = 0; i < rawPaths.length; i++) {
    ...
    // Prefer normalized payload if it’s valid
    let ref = normalizedPayload;
    if (!ref) {
      ref = (path.template_ref && path.template_ref !== "unknown")
        ? path.template_ref
        : (path.template_id && path.template_id !== "unknown")
          ? path.template_id
          : '';
    }

    console.log("exportFeatureBlockMessenger → path:", path);
    console.log("Resolved ref:", ref);
    ...
    console.log("exportFeatureBlockMessenger → path:", path);
    console.log("Looking for ref:", ref, "found node:", targetNode);
