...
export function FlowBuilderModule() {
  ...
  const onConnect_old = useCallback((params: Connection) => {
    console.log('🧩 onConnect triggered:', params);

    const sourceNode = nodes.find(n => n.id === params.source);
    let targetNode = nodes.find(n => n.id === params.target); // make mutable
    ...
    // 🪵 Debug logs
    console.log('🔎 onConnect type check:', {
    ...
    if (isFeatureBlock) {
      const targetType = targetNode.data?.block_type || targetNode.data?.blockType;
      if (targetType === 'carousel') {
        payload = targetNode.id;
      } else {
        payload = typeof targetNode.id === 'string'
          ? targetNode.id
    ...
    const updatedNodes = currentNodes.map(node => {
      ...
      if (node.id === targetNode.id && node.type === 'feature-block') {
        return {
          ...
          data: {
            ...node.data,
            block_type: 'carousel',   // ✅ consistent
            trigger: payload || node.data?.trigger,
            is_deep_target: true,
          },
        };
      }
      ...
      if (isQuickRepliesNode(sourceNode) &&
        ...
        if (replyIndex !== -1 && payload && isMessengerSafePayload(payload)) {
      ...
      const updatedCanvasPaths = safeCanvas.paths.map(path => {
        const match =
          ...
          path.targetBlockId === targetNode.id; // broaden match
        ...
        // 🔧 Use upgraded blockType
        const normalizedType = normalizeToTemplateType(
          targetNode.data?.blockType || targetNode.type
        );
        ...
        if (normalizedType === 'carousel') {
          ...
          payloadStatus = `✅ Linked directly to carousel block ${targetNode.id}`;
        }
        else if (normalizedType === 'feature-block') {
          autoLinkedPayload = {
            node_id: targetNode.id,
            template_type: 'feature-block',
            lang: 'en'
          };
          payloadStatus = `✅ Linked directly to feature-block ${targetNode.id}`;
        }
        else if (['text-message', 'generic-template', 'button-template', 'quick-replies', 'media-template', 'conversation-agent', 'voice-template'].includes(normalizedType)) {
          autoLinkedPayload = {
            node_id: targetNode.id,
            template_type: normalizedType,
            lang: 'en'
          };
          payloadStatus = `✅ Linked directly to ${normalizedType} node`;
        }
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    const payloadStr = `${targetType}.${targetNode.id}`;
    ...
    const updatedNodes = currentNodes.map(node => {
      ...
      const updatedCanvasPaths = safeCanvas.paths.map(path => {
        const match =
          ...
          path.sourceHandle === sourceHandle ||
          path.template_ref === sourceHandle ||
          path.targetBlockId === targetNode.id;

        if (!match) return path;

        console.log(`🧩 Path ${path.id} payload status: ✅ Linked to ${payloadStr}`);

        return {
          ...path,
          targetBlockId: target,
          payload: payloadStr, // ✅ string payload
        };
      });
      ...
      const updatedNodes = currentNodes.map(node => {
        ...
        if (node.type === 'feature-block') {
          ...
          const updatedPaths = paths.map(path => {
            ...
            const isValidTarget =
              targetNode &&
              (
                targetNode.type === 'text-message' ||
                targetNode.type === 'generic-template' ||
                targetNode.type === 'button-template' ||
                targetNode.type === 'quick-replies' ||
                targetNode.type === 'media-template' ||
                targetNode.type === 'conversation-agent' ||
                targetNode.type === 'voice-template'
              )
            ...
            if (isValidTarget) {
              ...
              if (safeType) {
                ...
                targetBlockId = targetNode!.id   // ✅ now we use the variable
                console.log(
                  `✅ Re-linked payload for path ${path.id} → node ${targetNode!.id} (${safeType})`
                )
              } else {
                updatedPayload = undefined
                targetBlockId = null
                console.warn(
                  `⚠️ Invalid template type for node ${targetNode!.id}: ${targetNode!.type}`
                )
              }
            }
            ...
            return {
              ...path,
              targetBlockId,   // ✅ use the updated variable
        ...
        if (isQuickRepliesNode(node)) {
          ...
          const updatedReplies = replies.map(reply => {
            ...
            if (payload && isMessengerSafePayload(payload)) {
              console.log(`✅ Re-linked payload for quick reply ${reply.id} → ${payload}`)
