  ...
  const handleAddConnectedNode = useCallback(
    (type: BuilderNodeType) => {
      ...
      switch (type) {
        ...
        case BuilderNode.CHAT_WITH_AGENT: safeType = "chat-with-agent"; break;
      ...
      // ✅ Inject payload into parent node depending on type
      ...
      if (fromNode.type === BuilderNode.FEATURE_BLOCK) {
        ...
        if (existingPath) {
          ...
          existingPath.payload = payloadStr;
          existingPath.template_ref = payloadStr;
        } else {
      ...
      // ✅ Commit edge (node already inserted by insertNode)
      setEdges([...getEdges(), simulatedEdge]);
