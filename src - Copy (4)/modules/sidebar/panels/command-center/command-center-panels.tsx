...
export function CommandCenterPanel() {
  ...
  const { setCurrentPageId } = useFlowSession()
...
const handleActivateWelcome = async (blockId: string) => {
  ...
  try {
    ...
    await set(welcomeRef, { activeBlockId: blockId })
     ...
     await fetch('https://9d8f-136-228-130-2.ngrok-free.app', {
      method: "POST",
          ...
          {selectedPageId && (
            <>
              ...
              <FeatureToggle
                ...
                path={`pages/${selectedPageId}/config/features/intent_prediction_messages`}
                tooltip="Enable AI to understand user inbox messages and predict their intent"
              />

              <FeatureToggle
                label="AI Smart Reply (Comments)"
                path={`pages/${selectedPageId}/config/features/intent_prediction_comments`}
