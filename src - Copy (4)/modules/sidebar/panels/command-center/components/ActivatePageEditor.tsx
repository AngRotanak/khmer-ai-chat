    ...
    const loaded = await Promise.all(
      Object.entries(data).map(async ([id, value]) => {
        ...
        if (autoStatus !== status) {
          const statusRefAdmin = ref(db, `khmer-ai-chat/admins/${user.id}/pages/${id}/status`)
          const statusRefPage = ref(db, `khmer-ai-chat/pages/${id}/status`)
          await Promise.all([set(statusRefAdmin, autoStatus), set(statusRefPage, autoStatus)])
        }
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`rounded px-2 py-0.5 text-xs ${
                    p.status === 'active' ? 'bg-green-700 text-white' :
