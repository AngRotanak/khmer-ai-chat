...
function getNodeTemplateId(node: Node): string {
  const d = node.data ?? {};
  return typeof d.template_id === 'string'
    ? d.template_id
...
//     const edges = Array.isArray((canvas as any).edges)
//       ? (canvas as any).edges.filter((e: any) => typeof e === 'object' && e !== null)
//       : fallbackEdges;

//     const paths = Array.isArray((canvas as any).paths)
//       ? (canvas as any).paths.filter((p: any) => typeof p === 'object' && p !== null)
//       : fallbackPaths;
